...
function bindUfoButton(id, dir){
    ...
    btn.addEventListener('pointerup',    ()=>{ if(ufoMoveDir === dir) ufoMoveDir = 0; });
    btn.addEventListener('pointerleave', ()=>{ if(ufoMoveDir === dir) ufoMoveDir = 0; });
    btn.addEventListener('pointercancel',()=>{ if(ufoMoveDir === dir) ufoMoveDir = 0; });
}
...
function getPlayerData(team){
    return team === 1 ? stateRef.p1 : stateRef.p2;
}
...
window.openMiniGame = function(team){
    ...
    // CPU 모드에서 P2 미니게임 차단
    if(stateRef.mode === 'cpu' && team === 2) return;
...
window.pullCapsule = function(){
    ...
    if(p.coin < MINI_COST){
        document.getElementById('mini-result').innerText = '코인이 부족합니다.';
        return;
    }
...
function getRandomReward(){
    ...
    let total = 0;
    MINI_REWARDS.forEach(r=>{ total += r.weight; });
...
window.pickMagicCard = function(index){
    ...
    if(p.cards.length >= MAX_CARDS){
        document.getElementById('magic-result').innerText = '카드 보유 한도입니다.';
        return;
    }

    if(p.coin < MINI_COST){
        document.getElementById('magic-result').innerText = '코인이 부족합니다.';
        return;
    }
    ...
    document.querySelectorAll('.magic-card').forEach((el, i)=>{
        ...
        el.innerText = i === index
            ? card.name + '\n획득!'
            : '카드 ' + (i + 1) + '\n?';
    });

    document.getElementById('magic-result').innerText = card.name + ' 카드 획득!';
...
function castDeleteCard(team){
    ...
    let fx = stateRef.scene.add.circle(target.x, target.y, 40, 0x000000, 0.8);
    ...
    let enemyTeam = team === 1 ? 2 : 1;
    showToast('P' + enemyTeam + '의 ' + targetName + ' 유닛이 사라졌습니다.');
}
...
function castReturnCard(team){
    ...
    let line = stateRef.scene.add.rectangle(1100, 150, 2000, 12, 0xffffff, 0.55);
    stateRef.scene.tweens.add({
        targets:line,
        alpha:0,
        duration:450,
    ...
    let enemyTeam = team === 1 ? 2 : 1;
    showToast('P' + enemyTeam + ' 유닛 ' + enemies.length + '명이 후퇴했습니다.');
}
...
function castStunCard(team){
    ...
    enemies.forEach(e=>{
        ...
        let fx = stateRef.scene.add.text(
            e.x, e.y - 50, 'STOP',
            {fontSize:'18px', color:'#99ccff', stroke:'#000', strokeThickness:3, fontStyle:'bold'}
        ).setOrigin(0.5);
    ...
    let enemyTeam = team === 1 ? 2 : 1;
    showToast('P' + enemyTeam + ' 유닛 ' + enemies.length + '명이 3초간 정지했습니다.');
}
...
function addHeroStock(team){
    ...
    if(p.heroes.length >= MAX_HEROES){
        return { success:false, name:'', message:'영웅 보유 한도입니다.' };
    }
    ...
    return { success:true, name:hero.name, message:hero.name + ' 영웅 획득!' };
}
...
function setUfoReadyMode(){
    ...
    if(area)    area.classList.add('ufo-ready');
...
function setUfoPlayingMode(){
    ...
    if(area)    area.classList.remove('ufo-ready');
...
window.startUfoGame = function(){
    ...
    document.getElementById('ufo-result').innerText = '버튼으로 이동하고 잡기를 눌러주세요!';
};
...
function resetUfoBoard(){
    ...
    if(area) area.querySelectorAll('.ufo-prize').forEach(e=>e.remove());
...
function createUfoPrizes(){
    ...
    for(let i = 0; i < 9; i++){
        ...
        let reward = UFO_REWARDS[Math.floor(Math.random() * UFO_REWARDS.length)];
        ...
        ufoPrizes.push({ x: x + 19, y: y + 19, reward, el });
    }
}
...
function dropUfoClaw(){
    ...
    document.getElementById('ufo-result').innerText = '집게가 내려갑니다...';
    ...
    stateRef.scene.time.delayedCall(520, ()=>{
        ...
        if(result.success){
            ...
            document.getElementById('ufo-result').innerText = '집게가 상품을 잡았습니다!';
        }else{
            claw.innerHTML = '╥<br>╩';
            document.getElementById('ufo-result').innerText = '상품을 놓쳤습니다...';
        }
        ...
        stateRef.scene.time.delayedCall(750, ()=>{
            ...
            if(result.success){
                ...
                document.getElementById('ufo-result').innerText = '실패! 다음 기회를 노려보세요.';
            }
...
function judgeUfoCatch(){
    ...
    ufoPrizes.forEach(p=>{
        ...
        if(d <= 36) candidates.push({ prize:p, dist:d });
    });

    candidates.sort((a,b)=>a.dist - b.dist);

    if(candidates.length === 0) return { success:false, best:null };

    let nearest = candidates[0];
    let chance  = 0;

    if(nearest.dist <= 6)       chance = 0.55;
    else if(nearest.dist <= 14) chance = 0.35;
    else if(nearest.dist <= 24) chance = 0.18;
    else                        chance = 0.08;

    let success = Math.random() < chance;

    if(!success) return { success:false, best:nearest.prize };

    let picked = nearest.prize;
    if(candidates.length >= 2 && Math.random() < 0.35) picked = candidates[1].prize;
    if(candidates.length >= 3 && Math.random() < 0.15) picked = candidates[2].prize;

    return { success:true, best:picked };
}
...
window.selectFishingGame = function(){
    ...
    document.getElementById('fish-result').innerText =
        '화면을 클릭하면 시작합니다. 찌가 초록 구간에 들어오는 순간 화면을 클릭하세요.';

    let practiceBtn = document.getElementById('fish-practice-success');

    if(practiceBtn){
        // 연습 모드에서만 연습 성공 버튼 표시
        practiceBtn.style.display =
            stateRef.mode === 'practice' ? 'inline-block' : 'none';
    }
...
window.startFishingGame = function(){
    ...
    if(p.coin < MINI_COST){
        document.getElementById('fish-result').innerText = '코인이 부족합니다.';
        return;
    }
    ...
    if(fishArea) fishArea.classList.remove('fish-ready');
    if(overlay)  overlay.style.display = 'none';
...
function resetFishingBoard(){
    ...
    if(target){ target.style.left = '220px'; target.style.width = '42px'; }
    if(cursor)  cursor.style.left = '0px';
    if(bite)    bite.style.display = 'none';
    if(area)    area.classList.add('fish-ready');
    if(overlay) overlay.style.display = 'flex';
...
function updateFishing(){
    ...
    if(Math.abs(diff) > 1){
        ...
        if(now >= fishHoldUntil) chooseNextFishGoal();
    }

    if(fishCursorX < 0)   fishCursorX = 0;
    if(fishCursorX > 494) fishCursorX = 494;
...
function chooseNextFishGoal(){
    ...
    let shouldBite = elapsed > 900 && Math.random() < 0.36;

    if(shouldBite){
        fishGoalX    = fishTargetX + Math.random() * Math.max(4, fishTargetW - 6);
    ...
    if(pattern < 0.35){
        ...
        fishGoalX = Math.random() < 0.5
            ? Math.max(0,   fishTargetX - 8 - Math.random() * 12)
            : Math.min(494, fishTargetX + fishTargetW + 4 + Math.random() * 12);
    }
...
window.forceFishSuccess = function(){
    ...
    if(!fishingActive){
        document.getElementById('fish-result').innerText = '낚시 시작 후 사용할 수 있습니다.';
        return;
    }
...
function finishFishing(success){
    ...
    if(success){
        ...
        document.getElementById('fish-result').innerText = result.message;
        showFishingResultImage(true);
    }else{
        document.getElementById('fish-result').innerText = '실패! 영웅을 놓쳤습니다.';
...
function showFishingResultImage(success){
    ...
    img.src    = success ? 'assets/fishing_success.png' : 'assets/fishing_fail.png';
...
function isFishBiting(){
    ...
    return (cursorCenter >= fishTargetX && cursorCenter <= fishTargetX + fishTargetW);
}
...
function updateFishBiteIndicator(){
    ...
    bite.style.display =
        (fishingActive && isFishBiting()) ? 'block' : 'none';
}
